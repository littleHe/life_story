...
define(["jquery", "miniMenu", "miniTheme", "miniTab"], function ($, miniMenu, miniTheme, miniTab) {
    ...
    // 暴露给 iframe 内容页：正文页（如 welcome.html 的快捷入口）内的 [layuimini-content-href]
    // 链接位于 iframe 文档中，父级 miniTab.listen 的点击代理绑定在父文档 body 上，无法捕获 iframe 内的点击。
    // 内容页需通过 parent.layui.miniTab.openNewTabByIframe(...) 主动在父级开新标签页，故此处把模块挂到 layui 上。
    if (typeof layui === 'object' && layui) {
        layui.miniTab = miniTab;
        layui.miniAdmin = miniAdmin;
    }
